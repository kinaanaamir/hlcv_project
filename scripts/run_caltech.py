...
if __name__ == "__main__":
    ...
    base_path = os.getcwd() + "/../datasets/caltech/"
    weight_path = os.getcwd() + "/../weights/"
    if not os.path.exists(weight_path):
        os.mkdir(weight_path)
    weight_path = weight_path + "caltech_weights/"
    if not os.path.exists(weight_path):
        os.mkdir(weight_path)

    train_dataloader, test_dataloader, total_classes = DataProcessingHelperMethods.prepare_caltech(
        base_path)
    ...
    with open(weight_path + "caltech_values.pickle", "wb") as handle:
