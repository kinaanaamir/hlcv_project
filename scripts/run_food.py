...
if __name__ == "__main__":
    ...
    base_path = os.getcwd() + "/../datasets/food_101/"
    weight_path = os.getcwd() + "/../weights/"
    if not os.path.exists(weight_path):
        os.mkdir(weight_path)
    weight_path = weight_path + "food_101_weights/"
    if not os.path.exists(weight_path):
        os.mkdir(weight_path)

    train_dataloader, test_dataloader, total_classes = DataProcessingHelperMethods.prepare_food_dataset(base_path)
    ...
    for dropout in dropouts:
        for model_names in all_models:
            ...
            final_name = final_name + str(dropout)
    ...
    with open(weights_path + "food_values.pickle", "wb") as handle:
