...
if __name__ == "__main__":
    path = "visualization/cub"
    single_model_path = glob.glob(os.path.join(path, "1_*"))[0]
    stacked_model_path = glob.glob(os.path.join(path, "4_*"))[0]
    ...
    read = lambda name: cv2.cvtColor(cv2.imread(name), cv2.COLOR_BGR2RGB)
    ...
    for image_path in glob.glob(os.path.join(single_model_path, "label_*_orig.png")):
        ...
        orig_image = read(image_path)
        single_pred_image = read(os.path.join(single_model_path, f"pred_{tag}_{single_backbone}.png"))
        single_label_image = read(os.path.join(single_model_path, f"label_{tag}_{single_backbone}.png"))
        ...
        plt.grid(False)
        plt.axis('off')
        axes[1][0].imshow(single_label_image)
        plt.grid(False)
        plt.axis('off')

        stacked_images = []
        for i, backbone in enumerate(stacked_backbones):
            tmp = read(os.path.join(stacked_model_path, f"{tag}_{backbone}.png"))
